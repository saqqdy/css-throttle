...
import { PACKAGE_PATH, ROOT_PATH, SOURCE_PATH } from '../utils/paths'
...
export const compileLess = task('compileLess', () =>
	src(resolve(SOURCE_PATH, 'style', '*.less'))
		.pipe(less())
		.pipe(
			autoprefixer({
				cascade: false
...
export const compileComponentLess = task('compileComponentLess', cb => {
	Object.keys(components).map(component =>
		src(resolve(SOURCE_PATH, component, 'style', '*.less'))
			...
			.pipe(dest(resolve(ROOT_PATH, 'lib', 'components', component, 'style')))
			.pipe(dest(resolve(ROOT_PATH, 'es', 'components', component, 'style')))
	)
	cb()
})
...
export async function buildStyleEntry() {
	Object.keys(components).map(async componentName => {
		const styleDir = resolve(SOURCE_PATH, componentName, 'style')
		const esDir = resolve(ROOT_PATH, 'es', 'components', componentName, 'style')
		const libDir = resolve(ROOT_PATH, 'lib', 'components', componentName, 'style')
...
export async function copyStyle() {
	const buildTasks = Object.keys(components).map(async component => {
		const baseStyle = resolve(SOURCE_PATH, 'style')
		const componentStyle = resolve(SOURCE_PATH, component, 'style')
		const baseEsOutput = resolve(ROOT_PATH, 'es', 'components', 'style')
		const baseLibOutput = resolve(ROOT_PATH, 'lib', 'components', 'style')
		const esOutput = resolve(ROOT_PATH, 'es', 'components', component, 'style')
		const libOutput = resolve(ROOT_PATH, 'lib', 'components', component, 'style')
		if (existsSync(`${componentStyle}/index.less`)) {
			await runSpawnSync(`mkdir -p ${esOutput} ${libOutput} ${baseEsOutput} ${baseLibOutput}`)
			await runSpawnSync(
		...
		await runSpawnSync(`rsync -a --include *.less --exclude * ${baseStyle}/ ${baseEsOutput}/`)
		await runSpawnSync(`rsync -a --include *.less --exclude * ${baseStyle}/ ${baseLibOutput}/`)
	})
...
export async function copyOutputStyle() {
	...
	await runSpawnSync(`mkdir -p ${baseOutput}`)
	await runExec(`rsync -a --include "*.css" --exclude "*" ${esStyle}/index.css ${baseOutput}/`)
}
